...
struct PositionPacket : public GamePacket {
	...
	PositionPacket(positionStruct tempPos) {
		type = BasicNetworkMessages::Position_Message;
		PlayerPosition.X = tempPos.X;
		PlayerPosition.Y = tempPos.Y;
		PlayerPosition.Z = tempPos.Z;
		size = sizeof(int)*3;
	};

	positionStruct GetPositionFromData() {
		positionStruct tempPosition;
		tempPosition.X = PlayerPosition.X;
		tempPosition.Y = PlayerPosition.Y;
		tempPosition.Z = PlayerPosition.Z;
		return tempPosition;
	}
};
...
struct TimePacket : public GamePacket {
	...
	TimePacket(const std::string& message) {
